<#
.SYNOPSIS
    Run VI Analyzer over a set of revisions using ONE warm Windows LabVIEW container.

.DESCRIPTION
    Windows-container "batch" counterpart of run-vi-analyzer-windows-container.yml.
    Instead of starting a fresh container per revision (the standard per-commit
    job), this starts a SINGLE long-lived container and analyzes each requested
    ...
    history that turns N×(startup+analyze) into startup+N×analyze.
    ...
    Reports are staged deploy-ready under:
        <OutRoot>\<sha>\index.html  raw.html  summary.json  [passes\]
    then published with keep_files semantics into gh-pages vi-analyzer/<sha>/.
...
param(
    ...
    # Explicit revisions to analyze (space / comma / newline separated, full or
    ...
    # 'vi-analyzer/<sha>/summary.json'). Used to skip done revisions.
    [string]$SkipListPath      = '',
...
if ($OutRoot -eq '') { $OutRoot = Join-Path $WorkspaceRoot 'ci-out\vi-analyzer-backfill' }
$OpsHost = Join-Path $WorkspaceRoot '.github\labview'

$TempRoot = if ($env:RUNNER_TEMP) { $env:RUNNER_TEMP } else { [System.IO.Path]::GetTempPath() }
$WorkTreesHost = Join-Path $TempRoot 'lvci-via-wt'
New-Item -ItemType Directory -Force -Path $OutRoot, $WorkTreesHost | Out-Null

$ContainerName = "lvci-via-$([System.Guid]::NewGuid().ToString('N').Substring(0,8))"
...
Write-Host "Revisions to analyze: $($Commits.Count)"
...
if (-not $Force -and $SkipListPath -ne '' -and (Test-Path $SkipListPath)) {
    foreach ($line in (Get-Content $SkipListPath)) {
        if ($line -match 'vi-analyzer/([0-9a-f]{7,40})/') { [void]$Done.Add($Matches[1]) }
    }
...
# (run-vi-analyzer.ps1 etc.) is mounted at C:\ops, and per-revision source
    ...
    foreach ($sha in $Commits) {
        ...
        try {
            ...
            & docker exec $ContainerName powershell -NonInteractive -ExecutionPolicy Bypass `
                -File 'C:\ops\run-vi-analyzer.ps1' `
                -WorkspaceRoot  "C:\wt\src-$sha" `
                -ReportDir      "$cOut" `
                -ConfigTemplate 'C:\ops\via-configs\via-config-default.viancfg' `
                -LabVIEWPath    $LabVIEWPath
            if ($LASTEXITCODE -ne 0) { Write-Warning "run-vi-analyzer returned $LASTEXITCODE for $short (continuing)." }
            ...
            # workflow does: a multi-config run leaves passes\manifest.json, otherwise
            # the native single-config index.html is the source.
            $passesDir = Join-Path $reportHostDir 'passes'
            $nativeIdx = Join-Path $reportHostDir 'index.html'
            $src = @()
            if (Test-Path (Join-Path $passesDir 'manifest.json')) { $src = @('--passes-dir', $passesDir) }
            elseif (Test-Path $nativeIdx)                          { $src = @('--in', $nativeIdx) }
            else {
                Write-Warning "No VI Analyzer report produced for $short (report not generated)."
                continue
            }
            ...
            & python (Join-Path $OpsHost 'build-analyzer-report.py') `
                @src `
                --out        $reportHostDir `
                --pages-base '../..' `
                --sha        $sha `
                ...
                --author     $aut `
                --date       $dat 2>$null
            if ($LASTEXITCODE -ne 0) { Write-Warning "report builder returned $LASTEXITCODE for $short (native report kept)." }

            if (-not (Test-Path (Join-Path $reportHostDir 'summary.json'))) {
                Write-Warning "No summary.json produced for $short."
                continue
            }
...
Write-Host ""
Write-Host "=== VI Analyzer backfill (Windows) complete: $processed analyzed, $skipped skipped ==="
exit 0
